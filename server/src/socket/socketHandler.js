...
export const setupSocketHandlers = (io) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('call:end', async ({ callId, peerId, durationSeconds, avgLatencyMs }) => {
      ...
      // Force end in DB just in case REST API fails or frontend drops
      try {
         await supabase.from('calls').update({ status: 'completed' }).eq('id', callId).in('status', ['initiating', 'in_progress']);
      } catch(e) {}
